<h1 align="center">Runway MCP Server</h1>

<p align="center">
  <strong>Create Runway AI tasks through RunAPI — 2 endpoints, 1 model — then poll status and check pricing. One MCP server, one API key.</strong>
</p>
...
`@runapi.ai/runway-mcp` is a focused MCP server for the **Runway** model line on RunAPI.
It lets an MCP-compatible assistant create Runway tasks, poll their status, and check current pricing — all through a single RunAPI API key.
...
| Tool | Auth | Purpose |
|---|---|---|
| `extend_video` | Yes | Create a Runway task (extend video) and optionally poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. |
| `text_to_video` | Yes | Create a Runway task (text to video) and optionally poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. |
| `get_task` | Yes | Fetch the current status and latest payload for an existing task. |
| `check_pricing` | No | Look up the current price for a Runway model and endpoint. |

---

## Models

Runway covers 1 model across 2 endpoints. Each tool accepts the models listed for it:
...
The assistant calls `check_pricing` to confirm the cost, then `extend_video`, and returns the task id, status, and output URLs.
