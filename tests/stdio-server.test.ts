...
describe("runway stdio MCP server", () => {
  ...
  it("exposes the model-line tools over the real stdio transport", async () => {
    ...
    client = new Client({ name: "runway-mcp-test", version: "0.1.0" });
    ...
    expect(names).toEqual(["check_pricing","extend_video","get_task","login","text_to_video"]);
    ...
    const multiEndpointModels: Record<string, string[]> = {"runway":["extend_video","text_to_video"]};
